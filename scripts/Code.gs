...
const DRUG_CATEGORY_MAP = {
  ...
  cardiovascular: [
    ...
    'corlanor', 'kevzara', 'vimpat', 'trulance',
  ],
  diabetes: [
...
function addToMailerLite(email, drug, source) {
  try {
    ...
    var response = UrlFetchApp.fetch('https://connect.mailerlite.com/api/subscribers', options);
...
function doPost(e) {
  try {
    ...
    try {
      ...
      sheet.appendRow([ts, email, drug, source, e.parameter['user-agent'] || '']);
    } catch (sheetErr) {
