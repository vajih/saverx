...
- Transactional email via Resend.com: welcome + day 3/7 follow-ups, 12 segmented templates
...
Update the Resend email sequences (already written in `docs/EMAIL_SEQUENCES.md` and deployed to `emails/`) to include affiliate CTAs in emails 2 and 3 of the GLP-1 sequence. Turn informational emails into a conversion funnel.
...
- [ ] Redeploy `scripts/Code.gs` as a new deployment in Apps Script editor
- [ ] Run `createHourlyTrigger()` in Apps Script editor
...
- [ ] Update email templates in `emails/` with affiliate CTAs (GLP-1 day 3 + day 7)
...
- [ ] Weekly: review Resend email delivery stats (resend.com dashboard)
...
| `scripts/Code.gs`                    | Google Apps Script — email capture + Resend delivery + follow-up queue |
| `emails/`                            | 12 HTML email templates (GLP-1, cardiovascular, diabetes, general)     |
